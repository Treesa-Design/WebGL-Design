...
export default function InteractiveFractalForest() {
  ...
  useEffect(() => {
    ...
    const appleGeo = new THREE.SphereGeometry(0.05, 12, 12);
    ...
    function makeLeafCluster(useLight) {
      ...
      for (let i = 0; i < count; i++) {
        ...
        const pos = new THREE.Vector3((Math.random() - 0.5) * 0.3, Math.random() * 0.3, (Math.random() - 0.5) * 0.3);
        ...
        if (Math.random() < 0.15) {
          ...
          apple.position.copy(
            pos.clone().add(new THREE.Vector3((Math.random() - 0.5) * 0.05, -0.02, (Math.random() - 0.5) * 0.05))
          );
    ...
    return () => {
      ...
      [trunkGeo, branchGeo, leafGeo, groundGeo, appleGeo].forEach((g) => g.dispose());
      [trunkMat, branchMat, leafMatDark, leafMatLight, groundMat, appleMat].forEach((m) => m.dispose());
